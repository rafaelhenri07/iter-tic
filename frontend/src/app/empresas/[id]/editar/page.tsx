...
import { fetchEmpresa, atualizarEmpresa } from "@/lib/api";
  ...
  return (
    <div className="min-h-[42px] w-full flex flex-wrap gap-1.5 items-center rounded-md border border-slate-200 bg-white px-3 py-2 shadow-sm transition focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-500/20 dark:border-slate-700 dark:bg-slate-800">
      ...
      <input
        ...
        onBlur={() => addTag(input)}
        placeholder={tags.length === 0 ? "Adicionar serviço..." : "Mais um serviço..."}
...
export default function EditarEmpresaPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const e = await fetchEmpresa(empresaId);
        ...
        setForm({
          ...
          telefone: e.telefone,
          email: e.email,
          servicos_ofertados: e.servicos_ofertados ?? [],
        });
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("success", `Empresa "${form.nome}" atualizada com sucesso!`);
      setTimeout(() => router.push("/empresas"), 1200);
    } catch (err) {
      showToast("error", err instanceof Error ? err.message : "Erro ao atualizar empresa.");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="space-y-2">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Digite o nome de um serviço e pressione{" "}
              <kbd className="rounded bg-slate-100 px-1 py-0.5 text-xs dark:bg-slate-700">Enter</kbd> ou{" "}
            ...
            {form.servicos_ofertados.length > 0 && (
              <p className="text-xs text-slate-400">
                {form.servicos_ofertados.length} serviço(s) cadastrado(s)
              </p>
            )}
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {submitting ? (
                  ...
                  <><Briefcase size={14} />Salvar Alterações</>
                )}
              </button>
            </div>
          </div>
