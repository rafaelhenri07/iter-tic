...
import { criarFornecedor, fetchCatalogo } from "@/lib/api";
...
export default function NovoFornecedorPage() {
  ...
  useEffect(() => {
    fetchCatalogo().then(setCatalogoItems).catch(() => {});
  }, []);

  const onSubmit = async (data: FornecedorFormData) => {
    setEnviando(true);
    try {
      await criarFornecedor(cleanFornecedorPayload(data));
      showToast("success", "Fornecedor criado com sucesso!");
      router.push("/fornecedores");
    } catch (e) {
      showToast("error", e instanceof Error ? e.message : "Erro ao criar fornecedor.");
    } finally {
      setEnviando(false);
    }
  };
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Preencha as informações para cadastrar um novo fornecedor na base do sistema.
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-background-card p-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2 mt-2 mb-6">
            ...
            <button
              type="button"
              onClick={() => append({ nome: "", telefone: "", email: "", cargo: "" })}
              className="flex items-center gap-1 text-xs font-semibold text-brand-primary hover:opacity-80 transition-colors uppercase tracking-wider"
            >
          ...
          {fields.length === 0 ? (
            <p className="text-sm text-foreground-muted text-center py-4">Nenhum contato adicionado.</p>
          ) : (
        ...
        <div className="rounded-xl border border-border bg-background-card p-6 space-y-4 shadow-sm">
          ...
          {catalogoItems.length === 0 ? (
            <p className="text-sm text-foreground-muted text-center py-4">
              Nenhum item no catálogo. <Link href="/catalogo" className="text-brand-primary hover:underline">Cadastre primeiro</Link>.
            </p>
          ) : (
            <MultiSelectCombobox
