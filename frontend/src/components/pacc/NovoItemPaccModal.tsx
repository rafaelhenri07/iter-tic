...
import { criarItemPacc, fetchAcoesPdticAtivas, fetchPeriodos } from "@/lib/api";
import { showToast } from "@/components/ui/Toast";
import type { PaccRevisao } from "@/types/pacc";
import type { PdticAcao } from "@/types/pdtic";

interface NovoItemPaccModalProps {
  exercicioId: number;
...
export function NovoItemPaccModal({
  exercicioId,
  revisoes,
  onClose,
  onSuccess,
}: NovoItemPaccModalProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      revisao_inclusao_id: revisoes[0]?.id,
      acao_pdtic_id: 0,
      numero_item: "",
      descricao_demanda: "",
      quantidade: "",
  ...
  useEffect(() => {
    async function loadAcoes() {
      try {
        ...
        // Se o backend está offline, usar array vazio
        setAcoesPdtic([]);
      } finally {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 px-6 py-5">
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border pt-4">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="flex h-9 items-center gap-2 rounded-lg bg-gradient-to-r from-teal-500 to-cyan-400 px-5 text-sm font-semibold text-white shadow-md transition-all hover:shadow-lg disabled:opacity-50"
            >
              {submitting ? (
