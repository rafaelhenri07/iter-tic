...
import { desativarAcaoPdtic } from "@/lib/api";
import { showToast } from "@/components/ui/Toast";

interface DesativarAcaoDialogProps {
  open: boolean;
  onClose: () => void;
  acao: PdticAcao;
...
export function DesativarAcaoDialog({
  open,
  onClose,
  acao,
  revisoes,
  onSuccess,
}: DesativarAcaoDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    setSubmitting(true);
    try {
      await desativarAcaoPdtic(acao.id, Number(selectedRevisaoId));
      showToast(
        "success",
        `Ação ${acao.codigo_acao} desativada com sucesso.`
      );
      onSuccess?.();
      onClose();
    } catch (err) {
      const msg =
        err instanceof Error ? err.message : "Erro ao desativar ação.";
      showToast("error", msg);
    } finally {
      setSubmitting(false);
    }
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-foreground-muted">
                Em qual revisão foi desativada? <span className="text-red-500">*</span>
              </label>
              <select
