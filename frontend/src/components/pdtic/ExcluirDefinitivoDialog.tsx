...
interface ExcluirDefinitivoDialogProps {
  open: boolean;
  onClose: () => void;
  acao: PdticAcao;
  onSuccess?: () => void;
...
export function ExcluirDefinitivoDialog({
  open,
  onClose,
  acao,
  onSuccess,
}: ExcluirDefinitivoDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      showToast(
        "success",
        `Ação ${acao.codigo_acao} excluída permanentemente.`
      );
      onSuccess?.();
      onClose();
    } catch (err) {
            ...
            <div className="rounded-xl border border-red-300 bg-red-50 p-4 dark:border-red-800/60 dark:bg-red-950/30">
              <div className="flex gap-3">
                ...
                <div className="text-sm text-red-700 dark:text-red-300 space-y-2">
                  <p>
                    <strong>Tem certeza que deseja apagar esta ação?</strong>
                  </p>
                  <p>
                  ...
                  <p className="text-red-600 dark:text-red-400">
                    Se a ação foi cancelada em uma revisão do PDTIC, utilize a opção{" "}
